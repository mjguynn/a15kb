...
cargo build --example a15kb-qml-plugin

echo "=== Installing QML plugin, this needs elevated permissions    ==="
echo "=== TODO: Is there another way to load a QML module from KDE? ==="
sudo mkdir -p $PLUGIN_DIR
sudo cp ./target/debug/examples/liba15kb_qml_plugin.so $PLUGIN_DIR/liba15kb_qml_plugin.so
