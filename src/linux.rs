...
const MAX_INT_FAN_SPEED: u8 = 229;
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
	    match &self.inner {
	    	...
	    	EK::EcAccessError { source } => {
	    		write!(f, "couldn't access embedded controller. caused by: {source}")
	    	}
	    	EK::UnsafeFanSpeed { speed } => {
	    		write!(
	    			f, 
	    			"unsafe fan speed {:.2}% specified (the minimum safe speed is {:.2}%)",
	    			speed*100.0,
	    			MIN_SAFE_FAN_SPEED*100.0
	    		)
	    	}
	    	EK::ImpossibleFanSpeed { speed } => {
	    		write!(f, "impossible fan speed {:.2}% specified", speed*100.0)
	    	}
	    	EK::InvalidBit(bit) => {
...
fn validate_fan_speed(speed: f32) -> Result<()> {
	if speed > 1.0 {
		Err(EK::ImpossibleFanSpeed { speed }.into())
	} else if speed < MIN_SAFE_FAN_SPEED {
		Err(EK::UnsafeFanSpeed { speed }.into())
	} else {
		Ok(())
	}
}
fn cvt_fan_speed(speed: f32) -> Result<u8> {
	...
	let int_speed = (MAX_INT_FAN_SPEED as f32 * speed).ceil() as u8;
	assert!(int_speed <= MAX_INT_FAN_SPEED);
	Ok(int_speed)
}
fn ivt_fan_speed(int_speed: u8) -> Result<f32> {
	let speed = int_speed as f32 / MAX_INT_FAN_SPEED as f32;
	validate_fan_speed(speed)?;
	Ok(speed)
}
